...
func TestNoClusterTLSPolicyRegisteredByDefault(t *testing.T) {
	// Guards the upstream ansible-operator-plugins default: without a
	// distribution registering an implementation (e.g. via a blank import
	// of an OpenShift-specific package), the hook must be a no-op.
